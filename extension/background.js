...
chrome.storage.onChanged.addListener((changes, area) => {
  ...
  const hasSyncChange = Object.keys(changes).some((key) =>
    ...
    chrome.storage.local.get([...syncKeys, "authUser"], (data) => {
          ...
          tabs.forEach((tab) => {
            chrome.scripting
              .executeScript({
                ...
                func: (payload) => {
                  window.postMessage(
                    { type: "FOCUS_SHIELD_EXT_UPDATE", payload },
                    "*",
                  );
                },
                args: [data],
      ...
      if (uid) {
        ...
        cloudSyncTimeout = setTimeout(() => {
          fetch("https://focusshield.vercel.app/api/sync", {
            method: "POST",
...
async function refreshFromCloud() {
  try {
    ...
    const response = await fetch(
      `https://focusshield.vercel.app/api/sync?uid=${encodeURIComponent(uid)}`,
    );
    ...
    if (Object.keys(toStore).length > 0) {
      ...
      console.log(
        "Cloud Sync: Updated local storage from Firestore",
        Object.keys(toStore),
      );
    }
  ...
  focusInterval = setInterval(async () => {
    ...
    if (timeLeft !== session.timeLeft) {
      ...
      chrome.action.setBadgeBackgroundColor({
        color: timeLeft > 0 ? "#6366f1" : "#10b981",
      });
    }
...
chrome.storage.local.get(["focusSession"], (data) => {
  if (
    data.focusSession &&
    data.focusSession.active &&
    !data.focusSession.paused
  ) {
    startFocusInterval();
  }
});
